...
import { Film, Star, Heart, Plus, Edit2, Trash2 } from 'lucide-react';
import { MyLifeMovie, MovieStatus } from '../../types';

interface MoviesSectionProps {
  movies: MyLifeMovie[];
  onAdd: () => void;
  onEdit: (movie: MyLifeMovie) => void;
...
export const MoviesSection: React.FC<MoviesSectionProps> = ({
  ...
  const [filter, setFilter] = useState<'all' | MovieStatus | 'favorites'>('all');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          >
            Quero assistir ({movies.filter((m) => m.status === 'want_to_watch').length})
          </button>
          <button
            type="button"
          ...
          >
            Assistidos ({movies.filter((m) => m.status === 'watched').length})
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span>Favoritos ({movies.filter((m) => m.favorite).length})</span>
          </button>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-14 px-4 bg-white/70 dark:bg-stone-900/60 rounded-3xl border border-stone-200/70 dark:border-stone-800/80">
          <Film className="w-10 h-10 mx-auto text-stone-400 dark:text-stone-600 mb-3" />
          ...
          <p className="text-xs text-stone-500 dark:text-stone-400 max-w-sm mx-auto mt-1 mb-4">
            Guarde recomendações de filmes para o fim de semana ou lembretes dos seus favoritos.
          </p>
          <button
            type="button"
            onClick={onAdd}
          ...
          >
            ...
            <span>Adicionar o primeiro filme</span>
          </button>
        </div>
      ) : (
                ...
                <div className="flex gap-3">
                  {movie.coverUrl ? (
                    ...
                    <div className="w-16 h-22 rounded-xl bg-[#F6F7F4] dark:bg-stone-800/80 border border-stone-200/80 dark:border-stone-700/80 flex flex-col items-center justify-center p-2 text-center shrink-0">
                      <Film className="w-6 h-6 text-[#1F3A34] dark:text-emerald-400 mb-1" />
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-1">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold border ${getStatusColor(movie.status)}`}>
                      ...
                      >
                        <Heart className={`w-4 h-4 ${movie.favorite ? 'text-rose-500 fill-rose-500' : ''}`} />
                      </button>
                    </div>
